...
void BaseEnemy::Render()
{
	...
	if (m_isDebugEnable) {
		...
		if (m_inRange)
		{
			SDL_SetRenderDrawColor(m_pRend, 250, 250, 0, 255);
		}
		else
		{
			SDL_SetRenderDrawColor(m_pRend, 250, 0, 250, 255);
		}
		...
		for (int t = 1; t < 360; t++)
		{
			...
			point2_x = m_playerDest->x + m_playerDest->w / 2;
			point2_y = m_playerDest->y + m_playerDest->h / 2;
